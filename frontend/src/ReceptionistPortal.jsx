...
function AftercareLinkBox({ visitId, copiedId, setCopiedId }) {
  const BASE_URL = window.location.origin; 
  const link = `${BASE_URL}/chat/${visitId}`;
  ...
  const copied = copiedId === visitId;

  return (
    <div
      onClick={(e) => e.stopPropagation()}
      style={{ marginTop: "0.75rem", background: "linear-gradient(135deg, #f0fdf4, #ecfdf5)", border: "1px solid #86efac", borderRadius: 12, padding: "1rem", display: "flex", flexDirection: "column", gap: "0.6rem" }}
    >
      <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
        <span style={{ fontSize: "1rem" }}>💬</span>
        <span style={{ color: "#15803d", fontWeight: 800, fontSize: "0.85rem", textTransform: 'uppercase', letterSpacing: '0.05em' }}>Aftercare Chat Ready</span>
      </div>

      <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", background: "white", border: "1px solid #d1fae5", borderRadius: 8, padding: "0.4rem 0.6rem" }}>
        <span style={{ flex: 1, fontSize: "0.78rem", color: "#065f46", fontFamily: "monospace", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{link}</span>
        <button onClick={copyLink} style={{ background: copied ? "#16a34a" : "#15803d", color: "white", border: "none", borderRadius: 7, padding: "0.4rem 0.8rem", fontSize: "0.75rem", fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", gap: 5 }}>
          {copied ? "✓ Copied!" : "📋 Copy"}
        </button>
      </div>

      <div style={{ color: "#166534", fontSize: "0.75rem", lineHeight: 1.5, marginBottom: '0.25rem' }}>
        Send this link to the patient. It will open their private AI recovery chat based on Dr.'s notes.
      </div>

      <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
        <a href={`https://wa.me/?text=${encodeURIComponent(`Hello! Your VitalsFlow aftercare chat is ready. Open it here to ask questions about your recovery and medicines: ${link}`)}`} target="_blank" rel="noreferrer" onClick={(e) => e.stopPropagation()} style={{ textDecoration: "none", background: "#25d366", color: "white", borderRadius: 8, padding: "0.5rem 1rem", fontSize: "0.8rem", fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flex: 1, gap: 6, boxShadow: "0 2px 4px rgba(37,211,102,0.2)" }}>
          ...
          <div className="r-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="r-modal-body">
              ...
              {selectedPatientView.bill?.status === 'paid' ? (
                <AftercareLinkBox visitId={selectedPatientView.id} copiedId={copiedId} setCopiedId={setCopiedId} />
              ) : (
                          ...
                          <div key={bill.id}>
                            ...
                            {isPaid && (
                              <div style={{ borderTop: "none", marginBottom: "1rem" }} onClick={(e) => e.stopPropagation()}>
                                <AftercareLinkBox visitId={isPaid} copiedId={copiedId} setCopiedId={setCopiedId} />
                              </div>
                            )}
                          </div>
                        );
