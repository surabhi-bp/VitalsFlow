...
function LoadingScreen() {
  return (
    <div style={{ height: "100dvh", background: "#050f1a", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "1.5rem", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <p style={{ color: "#475569", fontSize: "0.85rem", fontWeight: 500 }}>Syncing your medical records...</p>
    </div>
  );
}
...
export default function PatientChatPage(props) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (data.status !== "paid" && data.bills?.[0]?.status !== "paid") {
    ...
    if (visitId) load();
    else setError("Invalid secure link.");
  }, [visitId]);
  ...
  const handleSend = async (override) => {
    ...
    try {
      ...
      setIsTyping(false);
      setMessages(prev => [...prev, { id: Date.now() + 1, from: "hospital", type: "text", time: getTime(), text: "Connection error. Please call 📞 1800-VITALS." }]);
    }
  };
